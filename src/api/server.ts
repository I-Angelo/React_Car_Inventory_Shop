...
let token = '85107a5489c7a440d7db91a55fbc5ae93e0eeb3c6282eca7';
...
export const server_calls = {
    get: async () => { //here on the 'async' we are requesting data and not sending any variables inside the parentheses
        const response = await fetch('https://sweltering-thankful-stove.glitch.me/api/vehicles', {
            method: 'GET',
    ...
    update: async(id:string, data: any = {}) => { //we are passing an 'id' variable because we need to let the sapp know which contact we are 
                                            //updating
        const response = await fetch(`https://sweltering-thankful-stove.glitch.me/api/vehicles/${id}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-access-token': `Bearer ${token}`
